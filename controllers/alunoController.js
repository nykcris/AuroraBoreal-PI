...
class AlunoController {
  async alunos(req, res) {
    ...
    for (let i = 0; i < atividades.length; i++) {
      ...
      atividadesFeitas.push(
        respostas.length > 0
          ? { feito: 1, nota: respostas[0].nota }
          : { feito: 0, nota: 0 }
      );
    }

    res.render("Aluno/alunos_index", {
      layout: "layouts/layout",
      rows,
      atividades_cadastradas: atividades,
      atividadesFeitas,
    });
  }
  ...
  async responderAtividadesPost(req, res) {
    ...
    let filepath =
      "uploads/" + req.file.mimetype.split("/").pop() + "/" + req.file.filename;
    let file;
    if (req.file != null) {
      file = filepath;
    } else {
  ...
  async tabelaNotasFetch(req, res) {
    ...
    for (let i = 0; i < disciplinas.length; i++) {
      let notas = await DBN.obter(
        req.cookies.usuarioLogado,
        turma_disciplinas[i].id
      );
      data.push({
